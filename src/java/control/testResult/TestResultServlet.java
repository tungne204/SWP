...
public class TestResultServlet extends HttpServlet {
    ...
    private void listTestResults(HttpServletRequest request, HttpServletResponse response)
        ...
        if (recordIdParam != null && !recordIdParam.isEmpty()) {
            try {
                recordId = Integer.valueOf(recordIdParam);
        ...
        if (recordId != null) {
            testResults = testResultDAO.findByRecordId(recordId);
            totalRecords = testResults.size();
            totalPages = 1;
        } else {
            testResults = testResultDAO.getTestResults(searchQuery, testTypeFilter, page, PAGE_SIZE);
            totalRecords = testResultDAO.getTotalCount(searchQuery, testTypeFilter);
    ...
    private void showAddForm(HttpServletRequest request, HttpServletResponse response)
        ...
        List<String> testTypes = testResultDAO.getDistinctTestTypes();
        Integer preselectedRecordId = null;
        String recordIdParam = request.getParameter("recordId");
        if (recordIdParam != null && !recordIdParam.isEmpty()) {
            try {
                preselectedRecordId = Integer.parseInt(recordIdParam);
            } catch (NumberFormatException ignore) {
            }
        }
        ...
        request.setAttribute("testTypes", testTypes);
        request.setAttribute("preselectedRecordId", preselectedRecordId);
    ...
    private void insertTestResult(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String returnTo = request.getParameter("returnTo"); // "viewReport" hoặc null
            String recordIdForReturn = request.getParameter("recordId");

            if ("viewReport".equals(returnTo) && recordIdForReturn != null) {
                response.sendRedirect("medical-report?action=view&id=" + recordIdForReturn);
            } else {
