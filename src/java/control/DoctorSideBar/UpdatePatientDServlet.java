 ...
 * @author Kiên
 */
@WebServlet("/Update-PatientD")
public class UpdatePatientDServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (patient == null) {
                ...
                response.sendRedirect("Patient-ProfileD?pid=" + pid + "&error=notfound");
                return;
            }

            request.setAttribute("patient", patient);
            request.getRequestDispatcher("/receptionist/doctor/patientUpdateProfile.jsp").forward(request, response);

        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect("Patient-ProfileD?error=true");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.sendRedirect("Patient-ProfileD?pid=" + id + "&success=true");

        } catch (Exception e) {
            e.printStackTrace();
            // Nếu có lỗi thì quay lại trang profile và hiển thị thông báo lỗi
            String pid = request.getParameter("patientId");
            response.sendRedirect("Patient-ProfileD?pid=" + pid + "&error=true");
        }
    }
}
